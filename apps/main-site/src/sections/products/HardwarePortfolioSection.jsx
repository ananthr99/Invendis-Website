...
function ProductImage({ image, label }) {
...
export default function HardwarePortfolioSection({ data }) {
			...
			<div className="mx-auto max-w-screen-2xl">
				...
				<div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.5rem" }}>
					{items.map((item) => (
						...
						>
							<ProductImage image={item.image} label={item.name} />
							<h3 className="font-heading text-base font-bold text-brand-text">{item.name}</h3>
