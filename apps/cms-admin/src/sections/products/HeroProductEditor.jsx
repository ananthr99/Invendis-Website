...
export default function HeroProductEditor({ data, onChange }) {
	...
	async function handleFileSelect(file) {
        ...
        checkFileExists((d._pendingUploads ?? []).length, `apps/main-site/public/images/products/hero/${file.name}`);
    }
    ...
    function renamePendingUpload(idx, val) {
        ...
        checkFileExists(idx, `apps/main-site/public/images/products/hero/${val}`);
	}
	...
	return (
		<div>
			...
			<label className="admin-label" style={{ display: "block", marginTop: 16, marginBottom: 8 }}>
				Hero Images (auto-rotating carousel)
			</label>
			{imgs.length > 0 && (
				<div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 10 }}>
					{imgs.map((imgPath, idx) => (
						<div key={idx} style={{ position: "relative" }}>
							...
							<button onClick={() => removeImage(idx)} title="Remove" style={{ position: "absolute", top: -7, right: -7, width: 20, height: 20, borderRadius: "50%", background: "var(--admin-red)", color: "white", border: "none", cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
						</div>
					))}
				</div>
			)}
						...
						<div key={idx} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, padding: 8, background: "var(--admin-bg)", border: "1px solid var(--admin-border)", borderRadius: 6 }}>
							{previews[idx]?.previewUrl && <img src={previews[idx].previewUrl} alt="" style={{ height: 44, width: 68, objectFit: "cover", borderRadius: 4, flexShrink: 0 }} />}
                                ...
                                <p style={{ margin: 0, fontSize: 11, color: "var(--admin-muted)" }}>
                                    → /images/products/hero/{upload.filename}
                                </p>
