...
export default function HardwarePortfolioEditor({ data, onChange }) {
    ...
    function renamePendingUpload(i, val) {
        ...
        checkFileExists(i, `apps/main-site/public/images/products/hardware/${d.items[i]?.key || "_"}/${val}`);
	}

	function addItem() {
		onChange({ ...d, items: [...d.items, { key: "", name: "", description: "", tags: [], image: [] }] });
	}
	...
	return (
		<div>
			...
			<label className="admin-label" style={{ display: "block", marginTop: 16, marginBottom: 8 }}>Hardware Items</label>
			{d.items.map((item, i) => {
								...
								<div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, padding: 8, background: "var(--admin-bg)", border: "1px solid var(--admin-border)", borderRadius: 6 }}>
									...
									<div style={{ flex: 1, minWidth: 0 }}>
										...
										<p style={{ margin: 0, fontSize: 11, color: "var(--admin-muted)" }}>
											→ /images/products/hardware/{item.key || "…"}/{pending.filename}
										</p>
