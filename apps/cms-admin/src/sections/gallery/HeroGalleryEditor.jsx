...
export default function HeroGalleryEditor({ data, onChange }) {
	...
	async function handleFileSelect(file) {
		...
		checkFileExists(`apps/main-site/public/images/gallery/hero/${file.name}`);
	}

	function renamePending(val) {
		onChange({ ...d, _pendingUpload: { ...d._pendingUpload, filename: val } });
		checkFileExists(`apps/main-site/public/images/gallery/hero/${val}`);
	}
				...
				{pending && (
					<div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, padding: 8, background: "var(--admin-bg)", border: "1px solid var(--admin-border)", borderRadius: 6 }}>
						...
						<div style={{ flex: 1, minWidth: 0 }}>
							...
							/>
							<p style={{ margin: 0, fontSize: 11, color: "var(--admin-muted)" }}>→ /images/gallery/hero/{pending.filename}</p>
