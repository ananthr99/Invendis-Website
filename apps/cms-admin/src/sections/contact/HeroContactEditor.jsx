...
export default function HeroContactEditor({ data, onChange }) {
	...
	async function handleFileSelect(file) {
		...
		checkFileExists(`apps/main-site/public/images/contact/hero/${file.name}`);
	}

	function renamePending(val) {
		onChange({ ...d, _pendingUpload: { ...d._pendingUpload, filename: val } });
		checkFileExists(`apps/main-site/public/images/contact/hero/${val}`);
	}
			...
			<div className="admin-field" style={{ marginTop: 20, paddingTop: 20, borderTop: "1px solid var(--admin-border)" }}>
					...
					<div style={{ marginBottom: 12 }}>
						<div style={{ position: "relative", display: "inline-block" }}>
							<img
								src={rawUrl(d.image)}
								alt=""
								style={{ height: 80, width: 160, objectFit: "cover", borderRadius: 6, border: "1px solid var(--admin-border)", display: "block" }}
							/>
							<button
								onClick={removeImage}
								title="Remove"
				...
				{pending && (
					<div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, padding: 8, background: "var(--admin-bg)", border: "1px solid var(--admin-border)", borderRadius: 6 }}>
						{preview?.previewUrl && (
							<img src={preview.previewUrl} alt="" style={{ height: 52, width: 80, objectFit: "cover", borderRadius: 4, flexShrink: 0 }} />
						)}
						<div style={{ flex: 1, minWidth: 0 }}>
							...
							<p style={{ margin: 0, fontSize: 11, color: "var(--admin-muted)" }}>
								→ /images/contact/hero/{pending.filename}
							</p>
							{fileCheck === "checking" && (
				...
				<label style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", border: "1px solid var(--admin-border)", borderRadius: 8, cursor: "pointer", fontSize: 13, background: "white", fontFamily: "inherit" }}>
					...
					{pending ? "Replace image" : d.image ? "Replace image" : "Upload image"}
				</label>
			</div>
		</div>
	);
